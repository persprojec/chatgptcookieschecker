...
async def process_file(
    ...
    if not cookies:
        # new, more descriptive error for unrecognised cookie format
    ...
    if valid:
        # Email with fallback to "name"
        m_email = re.search(r'"email"\s*,\s*"([^"]+)"', js_payload)
        if m_email:
            candidate = m_email.group(1)
            if '@' in candidate:
                email = candidate
            else:
                m_name = re.search(r'"name"\s*,\s*"([^"]+)"', js_payload)
                email = m_name.group(1) if m_name else candidate
        else:
            m_name = re.search(r'"name"\s*,\s*"([^"]+)"', js_payload)
        ...
        # Expires on: month, day, year only
        m_exp = re.search(r'"subscriptionExpiresAt"\s*,\s*(\d+)', js_payload)
        if not m_exp:
            m_exp = re.search(r'subscriptionExpiresAt\\?",\s*(\d+)', html)
        if m_exp:
