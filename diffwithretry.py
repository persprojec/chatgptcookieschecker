...
def check_cookie(jar: MozillaCookieJar) -> tuple[dict,int]:
    ...
    for attempt in range(RETRIES):
        try:
            ...
            result["details"]["raw_session_response"] = r1.text
            break
    ...
    # ─── Step 2: /backend-api/subscriptions ────────────────────────────────────
    headers2 = {
        **HEADERS,
    ...
    for attempt in range(RETRIES):
        try:
            ...
            result["details"]["raw_sub_response"] = r2.text
...
async def process_one_file(
    ...
    if res["status"] == "success":
        ...
        err     = d.get("error","")
        sub_raw = d.get("raw_sub_response","")

        if err == "no accessToken":
            msg = "❌ This cookie is not working"
        elif "token_expired" in sub_raw:
            msg = "❌ This cookie is expired"
        elif "token_invalidated" in sub_raw:
            msg = "❌ This cookie session is expired"
        else:
            msg = "❌ This cookie is not working or expired"
